...
function cg(){
  usage() {
    ...
    echo -e "  \e[1;36m-l, --limit\e[0m     Set maximum output tokens (default: $limit)"
...
EOF
  ...
  while IFS= read -r line; do
    if [[ $line == data:* ]]; then
      ...
      local output_text=$(echo -n "$json_chunk" | jq -r '.candidates[].content.parts[].text')
      ...
      local formatted_output=$(echo -n "$formatted_output" | sed -E 's/\*/'"$(printf '\e[1;33m*\e[0m')"'/g')
      local formatted_output=$(echo -n "$formatted_output" | sed -E 's/`//g')  
